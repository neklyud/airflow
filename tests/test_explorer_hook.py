...
class TestExplorerHook:
    def setup_method(self):
        self.connection = Connection(host='https://api.test-explorer.io', password='apikey')
    ...
    @patch('requests.get', return_value=MockedResponse(status_code=200, json_data={'status': '0', 'message': '', 'result': ''}))
    ...
    @patch('requests.get', return_value=MockedResponse(status_code=200, json_data={'status': '1', 'result': 'success'}))
